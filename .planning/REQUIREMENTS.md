...
- [x] **INFRA-02**: React site built with Vite and deployed to Cloudflare Pages via wrangler

## v1.1 Requirements

Security hardening before sharing the URL with guests. Phases 6–8.
...
| Per-IP rate limiting (CF-03) | Workers.dev subdomain can't use free WAF rate limit rules; Workers Paid ($5/month) needed — not worth it for a 100-guest site; Cloudflare's built-in DDoS covers large floods for free |
| Worker proxy in front of R2 | Requires $5/month Workers Paid plan; cost-benefit unclear at 100-guest scale |
| Turnstile CAPTCHA | Over-engineering for a private wedding site; adds friction for real guests |
| Private bucket + signed URLs | High complexity; R2 zero-egress makes financial risk low; defer unless abuse observed |
| Re-running ingest / embed / cluster | Stages are correct and expensive (~3m20s CLIP); only resize + upload re-run needed |
